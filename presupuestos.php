...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($action) {
            ...
            case 'delete':
                $db->query(
                    "DELETE FROM presupuestos_items WHERE id = ? AND usuario_id = ?",
            ...
            case 'marcar_pagado':
                ...
                $item = $db->fetch(
                    "SELECT * FROM presupuestos_items WHERE id = ? AND usuario_id = ?",
                    [$_POST['id'], $_SESSION['user_id']]
                );
...
// Obtener items de presupuesto solo del usuario actual (para todos los usuarios)
...
// Obtener cuentas solo del usuario actual (para todos los usuarios)
$cuentas_usuario = $db->fetchAll("SELECT * FROM cuentas WHERE activa = 1 AND usuario_id = ? ORDER BY nombre", [$_SESSION['user_id']]);

// Estadísticas del mes actual solo del usuario actual (para todos los usuarios)
$mes_actual = date('Y-m');
$stats = $db->fetch(
    "SELECT 
